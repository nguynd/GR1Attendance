...
export default function StudentsInClass() {
  ...
  const handleDateSelect = async (date) => {
    ...
    const data = await getAttendanceByClassAndDate(id, date);
    const statusMap = new Map(
      data
        .filter((item) => item.student_id) // bỏ bản ghi null
        .map((item) => [item.student_id.toString(), item.status])
    );
              ...
              {currentStudents.map((sv, index) => (
                <tr key={sv.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 border-b text-center">
                    {selectedDate ? (
                      attendanceStatus.get(sv.student_id?.toString()) === true ? (
                      ...
                      <>
                        ...
                        {sv.total_sessions > 0
                          ? Math.round(
                              (sv.appearance / sv.total_sessions) * 100
                            )
                          : 0}
                        %)
                      </>
