...
export default function StudentsInClass() {
        ...
        <tbody>
          ${students
            .map(
              (sv) => `
            <tr>
              <td style="border:1px solid #ccc;padding:4px">${sv.name}</td>
              <td style="border:1px solid #ccc;padding:4px">${sv.student_id}</td>
              <td style="border:1px solid #ccc;padding:4px">${sv.birth_date}</td>
            </tr>
          `
            )
            .join("")}
  ...
  const result = await Swal.fire({
    title: `Xác nhận thêm ${students.length} sinh viên?`,
    html,
    showCancelButton: true,
 ...
 const handleImportExcel = async (e) => {
  ...
  reader.onload = async (event) => {
    try {
      ...
      setPreviewStudents(formatted); // 👉 hiển thị preview
