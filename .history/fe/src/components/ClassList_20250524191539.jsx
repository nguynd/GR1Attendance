...
export default function ClassList() {
  ...
  const handleSave = async (newClass) => {
    try {
      await axios.post("http://localhost:3001/api/classes", newClass);
      fetchData(); // reload danh sách lớp
    } catch (err) {
  ...
  return (
    <div className="p-6 ml-64 max-w-[calc(100vw-256px)]">
      ...
      {showModal && (
        <EditClass onClose={() => setShowModal(false)} onSave={handleSave} />
      )}
    </div>
  );
}
