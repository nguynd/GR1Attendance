  ...
  try {
    const result = await pool.query(
      ...
      [classId]
        ...
        console.log("📅 Dates từ DB:", result.rows);
...
// ✅ Lấy danh sách điểm danh theo lớp và ngày (ĐÃ SỬA)
const getAttendanceByClass = async (req, res) => {
  const classId = req.params.classId;
  const attendanceDate = req.query.date;

  // 🔍 In log kiểm tra backend nhận gì
  console.log("🧪 BACKEND nhận được:");
  console.log("classId =", classId, typeof classId);
  console.log("attendanceDate =", attendanceDate, typeof attendanceDate);
