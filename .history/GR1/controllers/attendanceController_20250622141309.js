...
const getAbsentDatesByStudent = async (req, res) => {
  ...
  if (!classId || !studentId) {
    return res.status(400).json({ error: "Thiếu classId hoặc studentId" });
  }
  ...
  try {
    ...
    const absentDates = result.rows.map(r => {
      const date = new Date(r.attendance_date.getTime() - r.attendance_date.getTimezoneOffset() * 60000);
      return date.toISOString().split('T')[0];
    });

    res.json(absentDates);
  } catch (err) {
    console.error("❌ Lỗi khi lấy danh sách buổi vắng:", err.stack);
...
const restoreAttendance = async (req, res) => {
  ...
  if (!classId || !studentId || !date) {
    return res.status(400).json({ error: "Thiếu classId, studentId hoặc date" });
  }
  ...
  try {
    const result = await pool.query(
      `UPDATE attendances
       SET status = TRUE
       WHERE class_id = $1 AND student_id = $2 AND attendance_date = $3
       RETURNING *`,
    ...
    res.json({ message: "Khôi phục điểm danh thành công", data: result.rows[0] });
  } catch (err) {
    console.error("Lỗi khi khôi phục điểm danh:", err);
