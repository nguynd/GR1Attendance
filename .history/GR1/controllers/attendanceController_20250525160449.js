...
const getAttendanceDatesByClass = async (req, res) => {
  ...
  try {
    ...
    const dates = result.rows.map(row => row.attendance_date.toISOString().split('T')[0]);
    res.json(dates);
  } catch (err) {
...
const initAttendanceByClass = async (req, res) => {
  ...
  try {
    // Kiểm tra nếu lớp này đã được điểm danh trong ngày
    ...
    // Insert điểm danh cho toàn bộ sinh viên trong lớp hôm nay
